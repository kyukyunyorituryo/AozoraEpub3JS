 ...
 * 章の最初の本文をsetChapterNameでセットしたらtrue */
  ...
  constructor(writer, jarPath) {
    ...
    // 拡張ラテン変換
    const latinPath = path.join(jarPath, "chuki_latin.txt");
    this.latinConverter = new LatinConverter(latinPath);
    ...
    lineNum = 0;
    for (let line of chukiSufFileLines) {
      lineNum++;
      if (line.length > 0 && line.charAt(0) !== '#') {
        const values = line.split('\t');
        // タグ取得 3列目は行末タグ
        ...
        this.sufChukiMap.set(values[0], tags);
        // 別名
        if (values.length > 3 && values[3].length > 0) {
          this.sufChukiMap.set(values[3] + values[0], tags);
        }
    ...
    // 外字フォント一覧取得
    const gaijiPath = this.writer.getGaijiFontPath(); // 仮定のパス
  ...
  getBookInfo(srcFile, src, imageInfoReader, titleType, pubFirst) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (!noRubyLine.startsWith('--------------------------------------------------')) {
          ...
          if (this.inComment) {
            ...
            if (this.commentLineNum > 20)
              LogAppender.warn(lineNum, `コメントが ${this.commentLineNum} 行 (${this.commentLineStart + 1}) -`);
            this.commentLineNum = 0;
            this.inComment = false;
            continue;
          } else {
            if (lineNum > 10 && !(this.commentPrint && this.commentConvert))
      ...
      if (this.addChapterName) {
        if (this.preChapterLineInfo === null) this.addChapterName = false; // 前の見出しがなければ中止
          ...
          if (name.length > 0) {
            ...
            this.preChapterLineInfo.lineNum = lineNum;
      ...
      while (m !== null) {
        ...
        if (this.chukiFlagPageBreak.has(chukiName)) {
          ...
          if (noRubyLine.length === m.index + chukiTag.length) {
            ...
            this.bookInfo.addChapterLineInfo(this.preChapterLineInfo);
            ...
            if (imageFileName !== null) {
              ...
              if (this.bookInfo.firstImageLineNum === -1) {
                ...
                if (imageInfo && imageInfo.width > 64 && imageInfo.height > 64) {
                  this.bookInfo.firstImageLineNum = lineNum;
      ...
      if (this.autoChapter && this.bookInfo.getChapterLevel(lineNum) === 0) {
        // 文字列から注記と前の空白を除去
      ...
      // 見出しの次の行＆見出しでない
      if (this.addNextChapterName === lineNum && this.bookInfo.getChapterLineInfo(lineNum) === null) {
        // 見出しの次の行を繋げる
        const name = this.getChapterName(noRubyLine);
        if (name.length > 0) {
    ...
    // 行数設定
    this.bookInfo.totalLineNum = lineNum;
  ...
  isPageBreakLine(line) {
    ...
    while (m) {
      return this.chukiFlagPageBreak.has(m[1]);
    }
    return false;
  }
  ...
  checkImageOnly(bookInfo, preLines, line, lineNum) {
    ...
    if (this.chukiFlagPageBreak.has(curChuki)) {
      // 2行前の行末が改ページまたは現在行が先頭から2行目
      if (!preLines[1] ||
        (preLines[1].indexOf('］') > 3 && this.chukiFlagPageBreak.has(preLines[1].substring(preLines[1].lastIndexOf('＃') + 1, preLines[1].length - 1)))
      ) {
        // 1行前が画像
        if (
  ...
  convertTextLineToEpub3(out, line, lineNum, noBr, noImage) {
    ...
    while ((m = this.chukiPattern.exec(line)) !== null) {
      ...
      if (tags != null) {
        ...
        // 改ページ注記
        ////////////////////////////////////////////////////////////////////
        if (this.chukiFlagPageBreak.has(chukiName) && !this.bookInfo.isNoPageBreakLine(lineNum)) {
          // 字下げ状態エラー出力
          if (this.inJisage >= 0) {
            LogAppender.warn(this.inJisage, '字下げ注記省略');
          ...
          // 改ページフラグ設定
          if (this.chukiFlagMiddle.has(chukiName)) {
            // 左右中央
            this.setPageBreakTrigger(this.pageBreakMiddle);
          } else if (this.chukiFlagBottom.has(chukiName)) {
            // ページ左
            this.setPageBreakTrigger(this.pageBreakBottom);
          } else if (this.bookInfo.isImageSectionLine(lineNum + 1)) {
            // 次の行が画像単ページの表紙
            if (this.writer.getImageIndex() === this.bookInfo.coverImageIndex && this.bookInfo.insertCoverPage) {
              // 先頭画像で表紙に移動なら改ページしない
        ...
        // 字下げフラグ処理
          ...
          if (tags.length > 1) this.inJisage = -1; // インライン
          else this.inJisage = lineNum; // ブロック開始
        ...
        if (!noTagAppend) {
          ...
          //行末で閉じる指定のタグをバッファに追加
          if (tags.length > 1) {
            bufSuf = tags[1] + bufSuf;  // bufSufの先頭にタグを挿入
          }
        }
        //ブロック注記の改行無しチェック
        if (this.chukiFlagNoBr.has(chukiName)) noBr = true;
      }
      else {
        // 画像 (訓点 ［＃（ス）］ は . があるかで判断)
 ...
 * //@param ch ルビ変換前の行文字列 */
  ...
  printGlyphFontTag(buf, gaijiFileName, className, baseChar) {
    const gaijiFilePath = path.join(this.writer.getGaijiFontPath(), gaijiFileName);
    // ファイルの存在を確認
  ...
  continue;
      ...
      if (i < end - 1 && ch[i + 1] >= 0xFE00 && ch[i + 1] <= 0xFE0F) {
        ...
        if (this.utf16FontMap != null && this.utf16FontMap.has(ch[i])) {
          ...
          if (gaijiFileName != null) {
            if (this.printGlyphFontTag(buf, gaijiFileName, "u" + ch[i].toString(16), "〓")) {
              LogAppender.info(lineNum, "外字フォント利用(IVS除外)", "" + ch[i] + "(" + gaijiFileName + ") -" + ch[i + 1].toString(16));
              i++; // IVSの次へ
              continue;
            }
          }
        ...
        if (this.printIvsBMP) {
          buf.push(ch[i]);
          buf.push(ch[i + 1]);
          LogAppender.info(lineNum, "IVSを出力します", "" + ch[i] + ch[i + 1] + "(u+" + ch[i].toString(16) + "+" + ch[i + 1].toString(16) + ")");
        } else {
          buf.push(ch[i]);
          LogAppender.info(lineNum, "IVS除外", ch[i] + "(u+" + ch[i].toString(16) + ") -" + ch[i + 1].toString(16));
        }
        i++; // IVSの次へ
        continue;
      }
          ...
          if (CharUtils.isHiragana(ch[i]) || CharUtils.isKatakana(ch[i]) || ch[i] === '〻') {
            ...
            if (this.dakutenType === 1 && !(this.inYoko || noTcy)) {
              ...
              if (this.printGlyphFontTag(buf, "dakuten/" + className + ".ttf", className, ch[i])) {
                LogAppender.info(this.lineNum, "濁点フォント利用", "" + ch[i] + ch[i + 1]);
                i++;
                continue;
              }
            }
